...
<% ReqNum = getRequestNumber(); %>	
	...
	{
		copyData();
		getValueSelectedEdit('portsVirtPortsEntry.asp','virtPort','500','450',target+"&SSID=<%get("_sid_");%>", document.virtPortForm.virtPortTable,'yes');
	}

	function addVirtualPort()
	{
		copyData();
		getSelectedAdd('portsVirtPortsEntry.asp','virtPort','500','450',target+"&SSID=<%get("_sid_");%>",'yes');
	}
